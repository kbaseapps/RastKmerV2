SERVICE = rastkmerv2
SERVICE_CAPS = RastKmerV2
SPEC_FILE = RastKmerV2.spec
URL = https://kbase.us/services/rastkmerv2
DIR = $(shell pwd)
LIB_DIR = lib
SCRIPTS_DIR = scripts
TEST_DIR = test
...
build:
	...
	mkdir -p $(LBIN_DIR)
	javac -cp $(LIB_DIR)/src -d $(LBIN_DIR) $(LIB_DIR)/src/KmerGutsJava.java
	echo 'java -Xmx2g -cp /kb/module/bin KmerGutsJava $$@' > /kb/deployment/bin/kmer_guts
	chmod +x /kb/deployment/bin/kmer_guts
